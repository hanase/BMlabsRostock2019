...
Download or clone this repository using for example 
```
...
# Assess uncertainty at present time
...
mu <- sqrt(sim17)
psi <- sqrt(sim30)
y <- sqrt(observed[,"HU2017"])
```

- Compute overall bias as $a = \frac{1}{IK} \sum_{i,k}(y_k - \mu_{ik})$:
    ```{r eval = eval}
a <- 1/(I*K) * sum(y - mu)
a
```
...
- Compute variance for each run as $\sigma^2_{i} = \frac{1}{K} \sum_{k}(y_k - a - \mu_{ik})^2$:
    ```{r eval = eval}
sigma.sq <- 1/K * apply((y - a - mu)^2, 2, sum)
sigma.sq
```
...
- Set propagation factors for the bias and variance. We assume that our model started in 2010. For simplicity, let's assume that we want to have our projections centered around the simulation results. Thus, we set the bias to 0.
...
- Having a distribution function available, one can extract any probability of interest. For example, derive the probability that Utah will have less than 1,250,000 housing units by 2030. We will use the `bmacdf()` function that returns the cumulative distribution function of a mixture of normal components at a given point:

    ```{r eval = eval}
bmacdf
bmacdf(sqrt(1250000), w, as.numeric(means30["Utah",]), var30)
